...
OUTPUT_FILE="scaling_benchmark_200.png"

echo "啟動擴展性報告生成 (1-200 範圍)..."
...
docker run --rm \
    ...
    bash -c "
        ...
        python3 /plot_scripts/plot_scaling_benchmark_200.py \
...
if [ $? -eq 0 ]; then
    echo
    echo "✓ 完成！擴展性報告 (1-200) 已保存到: $PROJECT_DIR/output_plots/$OUTPUT_FILE"
else
    echo
    echo "✗ 生成失敗"
    exit 1
