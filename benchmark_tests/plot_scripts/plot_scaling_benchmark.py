...
def plot_scaling_benchmark(results, output_file):
    ...
    if len(num_prompts_list) >= 3:
        log_x = np.log10(num_prompts_list)
        coeffs1 = np.polyfit(log_x, total_throughputs, 2)
        poly1 = np.poly1d(coeffs1)
        log_x_smooth = np.linspace(min(log_x), max(log_x), 300)
        x_smooth = 10 ** log_x_smooth
        y_smooth = poly1(log_x_smooth)
    ...
    # 設定 X 軸刻度 (對數尺度)
    ax1.set_xscale('log')
    ax1.set_xticks([1, 2, 5, 10, 20, 50, 100, 200, 500, 1000])
    ax1.set_xticklabels(['1', '2', '5', '10', '20', '50', '100', '200', '500', '1000'], rotation=0)
    ...
    if len(num_prompts_list) >= 3:
        log_x = np.log10(num_prompts_list)
        coeffs2 = np.polyfit(log_x, output_per_request, 2)
        poly2 = np.poly1d(coeffs2)
        log_x_smooth = np.linspace(min(log_x), max(log_x), 300)
        x_smooth = 10 ** log_x_smooth
        y_smooth = poly2(log_x_smooth)
    ...
    # 設定 X 軸刻度 (對數尺度)
    ax2.set_xscale('log')
    ax2.set_xticks([1, 2, 5, 10, 20, 50, 100, 200, 500, 1000])
    ax2.set_xticklabels(['1', '2', '5', '10', '20', '50', '100', '200', '500', '1000'], rotation=0)
    ...
    if len(num_prompts_list) >= 3:
        log_x = np.log10(num_prompts_list)
        coeffs3 = np.polyfit(log_x, ttfts, 2)
        poly3 = np.poly1d(coeffs3)
        log_x_smooth = np.linspace(min(log_x), max(log_x), 300)
        x_smooth = 10 ** log_x_smooth
        y_smooth = poly3(log_x_smooth)
    ...
    # 設定 X 軸刻度 (對數尺度)
    ax3.set_xscale('log')
    ax3.set_xticks([1, 2, 5, 10, 20, 50, 100, 200, 500, 1000])
    ax3.set_xticklabels(['1', '2', '5', '10', '20', '50', '100', '200', '500', '1000'], rotation=0)
    ax3.tick_params(axis='x', labelsize=9)

    # 添加總標題
    fig.suptitle('vLLM Scaling Benchmark - Performance vs Request Count (1~1000)',
