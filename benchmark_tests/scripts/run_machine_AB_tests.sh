...
# ============================================================
# Group 5: 20B + TP=2 (Config B only: 1 test)
# ============================================================
echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║  Group 5: 20B Model + TP=2 (Config B only)                ║${NC}"
...
for INPUT_LEN in 1024; do
    INPUT_LABEL="${INPUT_LEN:0:1}k"
    if "$BENCHMARK_SCRIPT" --config B --model 20B --tp 2 --input-len "$INPUT_LEN"; then
        PASSED_TESTS=$((PASSED_TESTS + 1))
    else
        FAILED_TESTS=$((FAILED_TESTS + 1))
