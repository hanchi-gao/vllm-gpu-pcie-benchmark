# AMD GPU 硬體測試手冊

> 快速驗證 AMD 硬體是否能正常運行 AI 模型

**專案 GitHub**: [https://github.com/hanchi-gao/vllm_t](https://github.com/hanchi-gao/vllm_t)

---

## 📋 手冊目的

當收到新的 AMD GPU 硬體時，使用本手冊快速驗證：

✅ **硬體是否正常工作**
✅ **能否成功載入 AI 模型**
✅ **模型推論是否穩定**
✅ **基本性能是否符合預期**

---

## 🎯 測試流程總覽

```
收到新硬體 → 環境設置 → 測試模型載入 → 執行壓力測試 → 收集結果 → 判定合格與否
   (5分鐘)     (10分鐘)      (5-10分鐘)       (1-3小時)      (查看檔案)   (根據數據)
```

---

## 📝 測試前準備

### 硬體資訊記錄

**在開始測試前，請記錄以下硬體資訊**：

```bash
# 查看 GPU 資訊
amd-smi

# 查看詳細規格
rocminfo | grep -E "Name|Marketing Name|gfx"
```

**記錄範例**：
- GPU 型號: AMD Radeon AI PRO R9700
- 架構代號: gfx1201
- GPU 數量: 4
- VRAM: 每卡 32GB

---

## 🚀 快速測試步驟

### 步驟 1: 啟動測試環境 (5 分鐘)

```bash
# 進入專案目錄
cd /path/to/vllm_t/docker_setup

# 啟動 Docker 容器
docker compose -f docker-compose.bench.yml up -d

# 確認容器運行
docker ps | grep vllm
```

**預期結果**：看到 `vllm-server` 和 `vllm-bench-client` 兩個容器在運行

---

### 步驟 2: 快速模型載入測試 (5-10 分鐘)

**目的**: 驗證硬體能否成功載入模型

#### 測試小型模型 (建議先測試)

**開啟終端 1**，進入服務器容器：

```bash
docker exec -it vllm-server bash
```

**測試 Gemma 3 4B** (輕量級模型，適合快速驗證)：

```bash
vllm serve google/gemma-3-4b-it \
  --tensor-parallel-size 1 \
  --gpu-memory-utilization 0.8 \
  --enforce-eager
```

**成功標誌**：
- 看到 `Loading model weights completed`
- 看到 `Application startup complete`
- 沒有錯誤訊息

**如果成功**：按 `Ctrl+C` 停止，繼續測試大型模型

#### 測試大型模型 (如果有多張 GPU)

```bash
vllm serve openai/gpt-oss-120b \
  --tensor-parallel-size 4 \
  --gpu-memory-utilization 0.9 \
  --enforce-eager
```

**成功標誌**：
- 模型成功載入
- 沒有 GPU 記憶體不足錯誤
- 沒有 ROCm 驅動錯誤

---

### 步驟 3: 執行壓力測試 (1-3 小時)

**目的**: 確認硬體在持續負載下穩定運行

保持終端 1 的 vLLM 服務器運行，**開啟終端 2**：

#### 選項 A: 快速驗證測試 (建議)

使用小型模型，測試時間較短：

```bash
docker exec vllm-bench-client bash -c \
  "cd /root && /root/benchmark_tests/scripts/run_scaling_bench_200.sh \
  --model google/gemma-3-4b-it"
```

**預估時間**: 30-60 分鐘

#### 選項 B: 完整壓力測試

使用大型模型，測試硬體極限：

```bash
docker exec vllm-bench-client bash /root/benchmark_tests/scripts/run_scaling_bench_200.sh
```

**預估時間**: 1-3 小時

---

### 步驟 4: 監控測試過程

**在測試過程中，開啟終端 3 監控硬體狀態**：

```bash
# 即時監控 GPU 使用率
watch -n 1 amd-smi

# 或在 Docker 內監控
docker exec vllm-server watch -n 1 amd-smi
```

**觀察重點**：
- GPU 使用率是否正常 (應該接近 100%)
- 溫度是否穩定 (不應持續上升)
- 記憶體使用是否穩定
- 是否有錯誤訊息

---

### 步驟 5: 收集測試結果

測試完成後，結果會儲存在：

```bash
/home/user/vllm_t/bench_results/scaling/{模型名稱}/
```

#### 查看結果檔案

```bash
# 列出所有結果檔案
ls -lh bench_results/scaling/*/

# 查看特定模型的結果
ls -lh bench_results/scaling/gemma-3-4b-it/

# 查看單一結果檔案
cat bench_results/scaling/gemma-3-4b-it/gemma-3-4b-it_scale_n10_*.json | python3 -m json.tool
```

---

## ✅ 判定標準

### 硬體合格標準

**✅ 通過測試** (硬體正常)：
- [ ] 模型成功載入，無錯誤訊息
- [ ] 完成所有 200 個測試點
- [ ] 生成 200 個 JSON 結果檔案
- [ ] 測試過程中無崩潰或重啟
- [ ] GPU 溫度穩定，未過熱
- [ ] 性能數據合理 (throughput > 0, latency 有數值)

**❌ 測試失敗** (硬體可能有問題)：
- [ ] 模型載入失敗
- [ ] 測試中途崩潰
- [ ] GPU 過熱保護觸發
- [ ] 大量錯誤訊息
- [ ] 結果檔案數量不足 200 個
- [ ] 性能異常低落

---

## 📊 關鍵指標說明

每個測試結果包含以下指標，用於判斷硬體性能：

| 指標 | 說明 | 參考值 |
|------|------|--------|
| **throughput** | 每秒處理請求數 | 越高越好，>0 表示正常 |
| **mean_ttft_ms** | 首 Token 時間 | 通常 < 1000ms |
| **mean_tpot_ms** | 每 Token 生成時間 | 越低越好 |
| **mean_latency_ms** | 平均延遲 | 取決於模型大小 |

**異常訊號**：
- throughput = 0 或非常低
- 延遲異常高 (>10000ms)
- 測試中斷或無結果

---

## 🔧 常見問題快速排查

### Q1: 模型載入失敗

**錯誤訊息**: `OutOfMemoryError` 或 `CUDA/HIP error`

**解決方法**：
1. 降低記憶體使用率：
   ```bash
   vllm serve google/gemma-3-4b-it \
     --enforce-eager \
     --gpu-memory-utilization 0.7  # 降至 70%
   ```

2. 使用更小的模型：
   ```bash
   vllm serve facebook/opt-125m \
     --enforce-eager \
     --tensor-parallel-size 1
   ```

### Q2: GPU 無法識別

**錯誤訊息**: `No GPU detected` 或 `ROCm not available`

**檢查步驟**：
```bash
# 檢查 GPU 是否被系統識別
amd-smi

# 檢查 Docker 是否能訪問 GPU
docker exec vllm-server amd-smi

# 檢查設備權限
ls -l /dev/kfd /dev/dri
```

### Q3: 測試中途停止

**可能原因**：
- GPU 過熱保護
- 記憶體不足
- 驅動不穩定

**檢查方法**：
```bash
# 查看系統日誌
dmesg | tail -50

# 查看 Docker 日誌
docker logs vllm-server | tail -100
```

---

## 🎯 建議測試模型清單

根據硬體規格選擇適合的測試模型：

### 單 GPU 測試 (VRAM < 32GB)

1. **Gemma 3 4B** - 基礎驗證
   ```bash
   vllm serve google/gemma-3-4b-it --enforce-eager
   ```

2. **Llama 3.1 8B** - 中等負載
   ```bash
   vllm serve meta-llama/Llama-3.1-8B --enforce-eager
   ```

3. **Qwen 3 8B** - 替代選項
   ```bash
   vllm serve Qwen/Qwen3-8B --enforce-eager
   ```

### 多 GPU 測試 (4+ GPUs)

4. **GPT-OSS 120B** - 重度壓力測試
   ```bash
   vllm serve openai/gpt-oss-120b \
     --tensor-parallel-size 4 \
     --enforce-eager
   ```

---

## 🔄 不同硬體的測試調整

### 針對不同 GPU 數量

| GPU 數量 | 建議測試模型 | tensor-parallel-size |
|---------|-------------|---------------------|
| 1 張 | Gemma 3 4B, Llama 3.1 8B | 1 |
| 2 張 | Llama 3.1 8B | 2 |
| 4 張 | GPT-OSS 120B | 4 |
| 8 張 | GPT-OSS 120B | 8 |

### 針對不同 VRAM 容量

| VRAM | 建議模型大小 | 記憶體使用率 |
|------|------------|-------------|
| 16GB | 4B-8B | 0.7 |
| 24GB | 8B-13B | 0.8 |
| 32GB+ | 8B-70B | 0.9 |
| 48GB+ | 70B-120B | 0.9 |

---

## 📚 相關資源

- **專案 GitHub**: [https://github.com/hanchi-gao/vllm_t](https://github.com/hanchi-gao/vllm_t)
- **完整技術文檔**: [README.md](README.md)
- **原始測試手冊**: [SCALING_200_GUIDE.md](SCALING_200_GUIDE.md)

---

## 🆘 聯絡支援

如測試過程遇到問題，請聯絡：

**VRD-Henry #11750**

提供以下資訊以便快速協助：
1. 硬體規格資訊 (`amd-smi`)
2. 錯誤訊息截圖
3. Docker 日誌: `docker logs vllm-server`
4. 系統日誌: `dmesg | tail -100`

---

**文檔更新日期**: 2024-12-24
**適用版本**: vLLM 0.10.2 + ROCm 7.0.0
